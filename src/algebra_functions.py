...
def intersects(segment_1, segment_2):
    ...
    """
    return counterclockwise(segment_1[0], segment_2[0], segment_2[1]) != counterclockwise(segment_1[1], segment_2[0],
                                                                                          segment_2[
                                                                                              1]) and counterclockwise(
        segment_1[0], segment_1[1], segment_2[0]) != counterclockwise(segment_1[0], segment_1[1], segment_2[1])
...
def angle_between(actor_from, actor_to):
    ...
    """
    if actor_to is None or actor_from is None:
        return 0
    ...
    alpha = np.degrees(np.arccos(np.dot(px, pg) / (np.linalg.norm(px) * np.linalg.norm(
        pg))))  # angle between actor_to, actor_from and x-axis (our angle origin)
...
def centroid(array):
    x_c = [el.coordinates[0] for el in array]
    y_c = [el.coordinates[1] for el in array]
    return [sum(x_c) / len(x_c), sum(y_c) / len(y_c)]
